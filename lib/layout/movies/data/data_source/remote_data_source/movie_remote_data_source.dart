...
import 'package:movie/layout/movies/data/models/movie_details_model.dart';
import 'package:movie/layout/movies/data/models/movies_model.dart';
import 'package:movie/layout/movies/data/models/recommendation_model.dart';
...
class MovieRemoteDataSource extends BaseMovieRemoteDataResource {
  @override
  Future<List<MovieModel>> getNowPlayingMovies() async {
    final response = await Dio()
        .get(ApiConstants.nowPlayingMoviePath);
  ...
  Future<MovieDetailsModel> getMovieDetails(MovieDetailsParameters parameters) async{
    final response = await Dio()
        .get(ApiConstants.movieDetailsPath(parameters.movieId));
    if (response.statusCode == 200) {
  ...
  Future<List<RecommendationModel>> getRecommendation(RecommendationParameters parameters)async {
    final response = await Dio()
        .get(ApiConstants.movieRecommendationPath(parameters.id));
    if (response.statusCode == 200) {
